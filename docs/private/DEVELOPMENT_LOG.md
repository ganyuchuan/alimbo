...
变更目标：
- 将 `src/gateway/gateway-server.ts` 中的 WebSocket 连接管理、消息分发和 cron 事件广播抽到独立模块，降低宿主文件复杂度。
- 统一 Claude / Copilot 的 hook 请求入口，改为通过 gateway 的 HTTP hooks 接口进行转发。
- 保持现有网关协议、工具路由和会话行为不变，只调整内部组织方式。

主要改动：
- Gateway 结构拆分
  - 新增：`src/gateway/http-server.ts`
    - 提供 gateway 的 HTTP 服务入口，承载健康检查与 hooks 路由。
  - 新增：`src/gateway/websocket-server.ts`
    - 承载 `/ws` 连接、connect 握手、copilot/git/sql/service/skills/mcp/cron 方法处理，以及 tick / cron.finished 事件推送。
  - `src/gateway/gateway-server.ts`
    - 退化为组合层，只负责装配 HTTP 与 WebSocket server，并在关闭时统一收口。
- HTTP hooks 接入
  - 新增：`src/gateway/http-hooks-common.ts`
  - 新增：`src/gateway/http-hooks-copilot.ts`
  - 新增：`src/gateway/http-hooks-claude.ts`
  - 通过 `src/gateway/http-server.ts` 对外暴露 `/api/hooks/pretool`、`/api/hooks/posttool`、`/api/hooks/session-start`、`/api/hooks/session-end`。
- hook 脚本适配
  - `hooks/scripts/*` 中的 Claude / Copilot hook 脚本统一改为请求 gateway HTTP hooks 接口。

验证记录：
- `npm run build`：通过
- `get_errors`（`src/gateway/gateway-server.ts`、`src/gateway/websocket-server.ts`）：无错误

## 2026-06-16

### 38) CLI 产品化重构：setup 拆分 + hooks 初始化/移除命令

变更目标：
- 将交互式 setup 流程拆分为参数化命令，降低首次接入和自动化部署门槛。
- 新增一键 hooks 初始化与回滚能力，支持全局安装后在任意仓库目录快速落地。
- 保持默认 `alimbo` 行为简洁：直接启动网关，并可通过 `--port` 覆盖端口。

主要改动：
- CLI 入口与命令分发
  - `src/cli.ts`
    - 默认 `alimbo` 路由到网关启动逻辑。
    - 新增子命令：`watch`、`feishu`、`init-hooks`、`unhook`。
    - `setup` 标记为 deprecated 并给出迁移提示。
- setup 拆分与共享能力
  - 删除：`src/setup.ts`
  - 新增：`src/cli/common.ts`
  ...
  - 新增：`src/cli/feishu.ts`
    ...
    - 写入飞书配置后确保 gateway 与 feishu 进程启动。
- hooks 初始化与回滚
  - 新增：`src/cli/init-hooks.ts`
    - 在当前终端目录生成 `.claude` 与 `.github/hooks` 配置和脚本。
    - 支持 `--force` 覆盖已有文件。
  - 新增：`src/cli/unhook.ts`
    - 从当前终端目录移除上述 hooks 产物，并清理空目录。
- 构建与分发
  - `package.json`
    - `postbuild` 增加 `hooks -> dist/hooks` 复制逻辑，保证全局安装场景可读取模板资源。

验证记录：
- `npm run build`：通过
- `node dist/cli.js --help`：通过
- `node dist/cli.js watch --help`：通过
- `node dist/cli.js feishu --help`：通过
- `node dist/cli.js init-hooks --force`：通过
- `node dist/cli.js unhook`：通过
...
变更目标：
- 将 Copilot/Claude runtime 中重复的 pretool、事件构造、token 状态机逻辑下沉为共享模块，降低双份维护成本。
- 新增 Claude Code 专用 hooks，并将原 Copilot hooks 迁移到独立目录，避免不同 hook 协议互相干扰。

主要改动：
- runtime 共享模块新增
  - `src/agent-runtime/pretool-gate.ts`
    - 统一 pretool 流程骨架：enabled 判断、工具命中、decision 标准化、fail-open 处理。
  - `src/agent-runtime/activity-event-builder.ts`
    - 统一 post tool 与 session lifecycle 事件构造。
  - `src/agent-runtime/token-event-builder.ts`
    - 统一 token estimate 事件构造。
  - `src/agent-runtime/session-token-tracker.ts`
    - 统一 tool token 统计、carryover 状态机与 request overhead 估算。
- runtime 接入重构
  - `src/agent-runtime/copilot.ts`
    - pretool 改为调用 `runPreToolInterceptGate`。
    - post/session/token 事件改为调用 builder。
    - token/carryover 改为 `sessionTokenTracker`。
  - `src/agent-runtime/claude.ts`
    - preToolHook 改为调用 `runPreToolInterceptGate`。
    - post/session/token 事件改为调用 builder。
    - token/carryover 改为 `sessionTokenTracker`。
- hooks 目录与配置调整
  - 新增 Claude 专用 hooks：
    - `scripts/hooks/claude/_common.mjs`
    - `scripts/hooks/claude/pretool.mjs`
    - `scripts/hooks/claude/posttool.mjs`
    - `scripts/hooks/claude/session-start.mjs`
    - `scripts/hooks/claude/session-end.mjs`
    - 支持 snake_case 输入字段（如 `tool_name/tool_input/session_id`）与 Claude pretool 输出结构。
  - Copilot hooks 迁移至：
    - `scripts/hooks/copilot/pretool.mjs`
    - `scripts/hooks/copilot/posttool.mjs`
    - `scripts/hooks/copilot/session-start.mjs`
    - `scripts/hooks/copilot/session-end.mjs`
  - 删除旧路径 Copilot hooks：
    - `scripts/hooks/pretool.mjs`
    - `scripts/hooks/posttool.mjs`
    - `scripts/hooks/session-start.mjs`
    - `scripts/hooks/session-end.mjs`
  - 配置更新：
    - `.github/hooks/alimbo-intercept.json` 指向 `scripts/hooks/copilot/*`
    - `.claude/settings.json` 指向 `scripts/hooks/claude/*`
- 文档同步
  - `README.md` TODO 区勾选：`Token 状态机`、`事件构造器`、`预工具拦截流程骨架`。

验证记录：
- `npm run build`：通过
- `printf '{"tool_name":"Bash","tool_input":{"command":"echo hi"},"session_id":"s1"}' | node scripts/hooks/claude/pretool.mjs`：返回 Claude pretool 决策结构
- `printf '{"tool_name":"Read","tool_input":{"file_path":"README.md"},"tool_response":{"ok":true},"session_id":"s1"}' | node scripts/hooks/claude/posttool.mjs`：通过
- `printf '{"toolName":"bash","toolArgs":{"command":"echo copilot"}}' | node scripts/hooks/copilot/pretool.mjs`：通过
- `printf '{"toolName":"read","toolArgs":{"path":"README.md"},"toolResult":{"ok":true}}' | node scripts/hooks/copilot/posttool.mjs`：通过

## 2026-06-02

### 31) 移除 blocked/restricted/destructive 工具策略配置

变更目标：
- 删除 `COPILOT_BLOCKED_TOOLS`、`COPILOT_RESTRICTED_DIR_TOOLS`、`COPILOT_DESTRUCTIVE_TOOLS` 及其相关策略逻辑，收敛 Copilot/Claude/runtime hooks 的权限处理面。
...
- 配置清理
  - `.env.example`、`.env`
    - 移除上述 3 个环境变量配置。
  - `src/config.ts`
    - 移除对应解析字段与 `COPILOT_ASK_BEFORE_DESTRUCTIVE` 依赖项。
- 运行时策略收敛
  - `src/agent-runtime/copilot.ts`
    - 删除 blocked/restricted/destructive 的判断分支与辅助默认列表。
  - `src/agent-runtime/claude.ts`
    - 删除同类策略分支与路径检查逻辑。
  - `scripts/hooks/_common.mjs`、`scripts/hooks/pretool.mjs`
    - 删除 hook 侧 blocked/restricted/destructive 的 helper 与拦截判断。
- 类型与文档同步
  - `src/tool/cron.ts`、`src/tool/sql.ts`
    - 移除对应 runtime config 类型字段。
  - `README.md`
    - 删除这 3 个参数及其策略说明/示例。

验证记录：
- `npm run build`：通过
- 全局检索确认：运行代码与文档中不再引用这 3 个参数，历史备份文件除外。

### 30) setup 切换 PM2 Node API + 修复 gateway 启动失败（io JSON 解析）

变更目标：
- 将 setup 的进程拉起逻辑切换为 PM2 Node API，彻底摆脱对全局 `pm2` 命令的依赖。
- 修复 setup 执行后 gateway 健康检查超时问题（PM2 内部 `JSON.parse(process.env.io)` 崩溃）。

主要改动：
- PM2 管理逻辑重构
  - `src/setup.ts`
    - 使用 `pm2.connect/describe/delete/start/disconnect` 管理 `alimbo-gateway` 与 `alimbo-feishu`。
    - 进程按固定名字幂等重建，避免多次 setup 导致 feishu 重复实例累积。
    - 输出 `processManager` 标识为 `pm2-api`。
- 启动失败修复
  - `src/setup.ts`
    - 在 PM2 `start` 的 `env` 中强制注入 `io: "{}"`，规避 `"undefined" is not valid JSON` 崩溃。
- 依赖补充
  - `package.json` / `package-lock.json`
    - 新增 `pm2` 依赖，确保项目内可直接使用 PM2 Node API。

验证记录：
- `npm run build`：通过
- 手动清理并重建 PM2 进程后，`alimbo-gateway` 状态为 `online`，不再出现 health check timeout。

### 29) intercept-server 默认 URL 统一到 go.aigc4me.cloud

变更目标：
- 将项目内 intercept 相关默认地址统一为 `https://go.aigc4me.cloud`，减少本地默认值与云端部署场景不一致导致的配置偏差。

主要改动：
- 默认值统一
  - `src/bridge/feishu.ts`
    - `createInterceptReviewClient` 的默认 `interceptServerUrl` 改为 `https://go.aigc4me.cloud`。
  - `src/setup.ts`
    - `.env` 模板中的 `COPILOT_INTERCEPT_SERVER_URL`、`FEISHU_INTERCEPT_SERVER_URL` 默认值改为 `https://go.aigc4me.cloud`。
    - setup 交互提示与 `cloudBaseUrl` fallback 默认值改为 `https://go.aigc4me.cloud`。
  - `src/config.ts`
    - `feishu.interceptServerUrl` 在环境变量缺失时的 fallback 改为 `https://go.aigc4me.cloud`。
  - `.env.example`
    - `FEISHU_INTERCEPT_SERVER_URL`、`COPILOT_INTERCEPT_SERVER_URL` 默认值改为 `https://go.aigc4me.cloud`。
- 文档同步
  - `README.md`
    - `FEISHU_INTERCEPT_SERVER_URL`、`CLOUD_SERVER_URL` 的默认值说明改为 `https://go.aigc4me.cloud`。

验证记录：
- 全局检索确认：intercept 默认值来源已切换到 `https://go.aigc4me.cloud`。
- 保留 `README` 中本地健康检查 `http://127.0.0.1:18790/health` 示例，不作为默认值配置来源。

### 28) Copilot CLI hooks 对齐 copilot runtime + 文档目录忽略收口

变更目标：
- 将 `.github/hooks/alimbo-intercept.json` 调用的 4 个 Node hooks 脚本与 `src/agent-runtime/copilot.ts` 的 `onPreToolUse/onPostToolUse/onSessionStart/onSessionEnd` 行为尽量对齐。
- 保持 hooks 输出稳定（stdout 仅返回 JSON），并将运行日志统一导向 stderr 便于排查。
- 收敛 docs 跟踪策略：`docs/` 目录统一忽略，历史已跟踪文档从 git 索引移除。

主要改动：
- hooks 对齐
  - `scripts/hooks/_common.mjs`
    - 新增规则解析与集合工具：`blocked/restricted/destructive/intercept/allowedDirs`。
    - 新增路径候选提取、目录白名单校验、hint 生成、toolArgs 脱敏、session entries 收集。
    - 新增生命周期状态持久化（`~/.copilot/hooks/lifecycle-state.json`），用于 session start/end 状态同步。
    - `dotenv` 加载改为 `quiet`，避免污染 hooks JSON 输出。
  - `scripts/hooks/pretool.mjs`
    - 行为顺序对齐 `onPreToolUse`：intercept 决策 -> blockedTools -> allowedDirs/restricted -> askBeforeDestructive -> allow。
    - 复用 `requestInterceptDecisionByApi`，并在失败时遵循 `COPILOT_INTERCEPT_FAIL_OPEN`。
  - `scripts/hooks/posttool.mjs`
    - 事件结构对齐 `reportPostToolUseEvent`：补齐 `prompt` 与 `toolCall.id`，并对 args/result 脱敏上报。
  - `scripts/hooks/session-start.mjs`、`scripts/hooks/session-end.mjs`
    - 事件结构对齐 session lifecycle 上报：补齐 `state/entries/prompt/session` 字段。
- hooks 配置切换
  - `.github/hooks/alimbo-intercept.json`
    - 从 bash+curl 长命令切换为 Node 脚本调用。
  - `~/.copilot/hooks/alimbo-intercept.json`
    - 同步为仓库配置版本。
- docs 忽略策略
  - `.gitignore`
    - 保持 `docs/` 忽略规则生效。
  - 从索引移除：`docs/CLOUD_AUTH_API.md`、`docs/DEVELOPMENT_LOG.md`（保留本地文件）。

验证记录：
- `printf '{}' | node scripts/hooks/pretool.mjs`：通过
- `printf '{...}' | node scripts/hooks/posttool.mjs`：通过
- `printf '{...}' | node scripts/hooks/session-start.mjs`：通过
- `printf '{...}' | node scripts/hooks/session-end.mjs`：通过
- hooks 脚本文件诊断：无错误
...
变更目标：
- 为 alimbo 增加基础 git 远程执行能力，支持通过 gateway `git` 方法和飞书 `/git` 命令在当前目录执行 allowlist 内的 git 指令。
...
- `src/config.mjs`
  ...
  - 新增 `SERVICE_WHITELIST`（逗号分隔），默认值为 `alimbo-gateway,alimbo-feishu`。
- `src/gateway/server.mjs`
...
- `npm run build`：通过
- `./node_modules/.bin/pm2 restart 1`：进程 alimbo-feishu 重启成功，状态 online
