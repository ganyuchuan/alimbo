...
function sendEventFrame(socket: any, event: string, payload: any) {
...
function shouldPushCronEventToConnection(connId: string, state: any, notify: any) {
  if (!state.connected) {
    return false;
  }
...
export function createGatewayWebSocketServer(config: any, { httpServer, cronScheduler }: { httpServer: any; cronScheduler?: any } = { httpServer: undefined, cronScheduler: undefined }) {
  const connections = new Map<string, any>();

  const broadcastCronFinishedEvent = ({ job, trigger, status, error, output }: any) => {
    const payload = {
      ts: Date.now(),
      job: {
        id: job?.id,
  ...
  const unsubscribeCronFinished =
    cronScheduler && typeof cronScheduler.onJobFinished === "function"
      ? cronScheduler.onJobFinished((event: any) => {
  ...
  const unauthorized = (id: any) => makeResponse(id, false, undefined, makeError("UNAUTHORIZED", "invalid token"));
  const badRequest = (id: any, message: string) => makeResponse(id, false, undefined, makeError("INVALID_REQUEST", message));
    ...
    socket.on("message", async (buffer: Buffer) => {
      ...
      try {
        ...
        if (frame.method === "copilot") {
          ...
          const { output, sessionId } = await runAgentWithSharedSession({
            ...
            onDelta: stream
              ...
              : undefined,
            onDone: stream
              ? ({ output: finalOutput, sessionId: finalSessionId }: any) => {
          ...
          socket.send(JSON.stringify(makeResponse(frame.id, true, { output, sessionId: sessionId || undefined })));
          return;
        }
        ...
        if (frame.method === "git") {
          ...
          socket.send(JSON.stringify(makeResponse(frame.id, true, { ...result, allowedCommands: config.git.allowedCommands })));
          return;
        }
